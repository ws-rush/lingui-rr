...
const defaultIgnorePaths: Array<RegExp | string> = [
  /^\/assets\//,
  /^\/build\//,
  /^\/favicon\.ico$/,
  /^\/robots\.txt$/,
...
export function isIgnoredPath(
  pathname: string,
  ignorePaths: Array<RegExp | string> = defaultIgnorePaths,
): boolean {
...
export function rewriteLocalePath(
  ...
  if (isIgnoredPath(url.pathname, options.ignorePaths ?? defaultIgnorePaths))
    return `${url.pathname}${url.search}${url.hash}`

  const target = matchSupportedLocale(
    targetLocale,
    supportedLocales,
    options.defaultLocale ?? supportedLocales[0],
  )
  ...
  const firstSupported = first
    ? matchSupportedLocale(first, supportedLocales, '')
    : ''
